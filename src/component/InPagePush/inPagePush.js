...
export default function InPagePush({ setAdOption }) {
    const data = useRef(null)
    const { register, setValue, getValues } = useForm()
    
    const [blockType, setBlockType] = useState([])

    const [adLabel, setAdLabel] = useState(adLabels[0].id)
    const [adPosition, setAdPosition] = useState(adPositions[0].value)
    
    const getOptionValue = () => {
        let data = getValues()
        data.blockType = blockType
        data.adLabel = adLabel
        data.adPosition = adPosition
        return data
    }
    data.current = getOptionValue()
   /*  setAdOption(data.current) */
    // ad review
    const [adReviewDevice, setAdReviewDevice] = useState(adReviewDevices[0].id)
    
    {/* ********************************* */}
    /*  phan 2 */
    const [adLabel2, setAdLabel2] = useState(adLabels[0].id)
    
    const [adPosition2, setAdPosition2] = useState(adPositions[0].id)
    
    const [adReviewDevice2, setAdReviewDevice2] = useState(adReviewDevices[0].id)

    const [imageDesktop2, setImageDesktop2] = useState(imageDesktop[0].id)
    
    
    return (<>
        <section className="grid lg:grid-cols-2 md:grid-cols-1 gap-6 bg-gray-900 p-10">
            <section className="space-y-7">
                {/* Ad Label */}
                <div className="md:flex md:items-center w-full">
                    <div className="md:w-1/4">
                        <label className="block text-gray-500 dark:text-gray-200 mb-3 md:mb-0 pr-4">Ad Label</label>
                    </div>
                    <div className="w-full">
                        <div className="space-x-5">
                            <RadioGroup value={adLabel} onChange={setAdLabel}>
                                {adLabels.map(item => (<>
                                    <RadioGroup.Option
                                        as="span"
                                        key={item.id}
                                        value={item.id}
                                        className={({ checked }) => `${checked ? 'text-white bg-purple-500' : 'text-purple-500 border-purple-500'} ${item.buttonLeft ? 'rounded-l' : ''} ${item.buttonRight ? 'rounded-r' : ''} cursor-pointer border border-purple-500 font-bold text-xs px-4 py-2 outline-none focus:outline-none ease-linear transition-all duration-150`}
                                    >
                                        {item.name}
                                    </RadioGroup.Option>
                ...
                <div className="md:flex md:items-center w-full">
                    <div className="md:w-1/4">
                        <label className="block text-gray-500 dark:text-gray-200 mb-3 md:mb-0 pr-4">Ad Position</label>
                    </div>
                    <div className="w-full">
                        <div className="space-x-5">
                            <RadioGroup value={adPosition} onChange={setAdPosition}>
                                {adPositions.map(item => (
                                    <RadioGroup.Option
                                        as="span"
                                        key={item.id}
                                        value={item.value}
                                        className={({ checked }) => `${checked ? 'text-white bg-purple-500' : 'text-purple-500 border-purple-500'}
                                            ${item.buttonLeft ? 'rounded-l' : ''}
                                            ${item.buttonRight ? 'rounded-r' : ''}
                                            cursor-pointer border border-purple-500 font-bold text-xs px-4 py-2 outline-none focus:outline-none ease-linear transition-all duration-150`
                                        }
                                    >
                                        {item.name}
                                    </RadioGroup.Option>
                ...
                <div>
                    <div className="md:flex md:items-center w-full mb-6">
                                                        <div className="md:w-1/12">
                                                            <label className="block text-gray-500 dark:text-gray-200 mb-3 md:mb-0" htmlFor="mobileImage">Image</label>
                                                        </div>
                                                        <div className="flex flex-row w-full">
                                                            {
                                                                imageDesktop.map((item)=>(
                                                                    <div key={item.id}>
                                                                        <button type="button" className={`${imageDesktop2===item.id ? `p-3 bg-indigo-700 text-white `: `p-3 bg-indigo-300 text-white`} p-3 placeholder-gray-400 text-gray-800 `} type="button"
                                                                        onClick={()=>setImageDesktop2(item.id)}
                                                                        >{item.id}</button>
                                                                    </div>
                                                                ))
                                                            }
                                                            {imageDesktop2 === 'Ratio' &&
                                                                <div className="md:w-full">
                                                                    <select  className="px-3 py-3 placeholder-gray-400 text-gray-800  bg-white rounded-r-lg border-0 shadow outline-none focus:outline-none focus:ring w-full" name="mobileImage" id="mobileImage" >
                                                            ...
                                                            {imageDesktop2 === 'Fixed' &&
                                                                <div className="md:w-full">
                                                                    <span className='p-3 bg-gray-300 text-black'>W</span>
                                                                    <input className='px-3 py-3 placeholder-gray-400 text-gray-800  bg-white rounded-r-lg border-0 shadow' type='number' />
                                                                    <span className='p-3 bg-gray-300 text-black'>H</span>
                                                                    <input className='px-3 py-3 placeholder-gray-400 text-gray-800  bg-white rounded-r-lg border-0 shadow' type='number' />
                                                                </div>
                                                            }
                                                        </div>
            ...
            <section className="flex flex-col items-center space-y-7">
                <div className="">
                    <RadioGroup value={adReviewDevice} onChange={setAdReviewDevice}>
                        {adReviewDevices.map(item => (
                            <RadioGroup.Option
                                as="span"
                                key={item.id}
                                value={item.id}
                                className={({ checked }) => `${checked ? 'text-white bg-green-500' : 'text-green-500 border:green-500 bg-white'} ${item.buttonLeft ? 'rounded-l-3xl border-r' : ''} ${item.buttonRight ? 'rounded-r-3xl border-l' : ''} cursor-pointer inline-flex justify-center font-bold px-4 py-2 outline-none focus:outline-none ease-linear transition-all duration-150`}
                            >
                                {item.icon}
                                {item.name}
                            </RadioGroup.Option>
                ...
                <div className="w-full">
                    {adReviewDevices.map(item => (
                        <Fragment key={item.id}>
                            {adReviewDevice === item.id &&
                                <div
                                    className="relative align-middle bg-gray-50"
                                    style={{
                                        border: '2px solid #e1e4eb',
                                        borderRadius: '10px',
                                        borderWidth: `${item.id === 'mobile' ? '48px 8px 24px' : '10px 5px'}`,
                                        minHeight: '600px',
                                        margin: '0 auto',
                                        maxWidth: item.id === 'mobile' && 376
                                    }}
                                >
                                    {item.iframe}
                                    <IppDemo device={item.id} position={adPosition} />
                                </div>
                            }
                        </Fragment>
                    ))}
        ...
        {/* ********************************* */}
            {/* Phần 2 */}
        <section className='bg-gray-900 grid grid-cols-2'>
            <div>
                <div>
                        {
                            adLabels.map((item)=>{
                                return <>
                                    <button key={item.id} type='button' 
                                        className={adLabel2===item.id ? `p-3 mx-5 rounded-lg bg-indigo-700 text-white ring ring-indigo-700 ring-offset-2`: `p-3 m-1 rounded-lg bg-indigo-300 text-white ring ring-indigo-300 ring-offset-2`}
                                        onClick={()=>{setAdLabel2(item.id)}}
                                    >{item.name}</button>
                                </>
                ...
                <div className='my-10'>
                        {
                            adPositions.map((item)=>{
                                return <>
                                    <button key={item.id} type='button' 
                                        className={adPosition2===item.id ? `p-3 mx-5 rounded-lg bg-indigo-700 text-white ring ring-indigo-700 ring-offset-2`: `p-3 m-1 rounded-lg bg-indigo-300 text-white ring ring-indigo-300 ring-offset-2`}
                                        onClick={()=>{setAdPosition2(item.id)}}
                                    >{item.name}</button>
                                </>
                ...
                <div className='flex justify-center '>
                        {
                            adReviewDevices.map((item)=>(
                                    ...
                                    <button key={item.id} type='button' 
                                        className={
                                            `${adReviewDevice2===item.id ? `p-3 bg-indigo-700 text-white `: `p-3 bg-indigo-300 text-white`}
                                            ${item.buttonLeft ? 'rounded-l-3xl border-r' : ''} 
                                            ${item.buttonRight ? 'rounded-r-3xl border-l' : ''}
                                            flex items-center`    
                                    }
                                        onClick={()=>{setAdReviewDevice2(item.id)}}
                                    >
                                        {item.icon}
                                        {item.name}
                                    </button>
                ...
                <div className='w-full relative'>
                        {
                            adReviewDevices.map((item)=>(
                                <div key={item.id} className="mt-5">
                                    {adReviewDevice2===item.id &&
                                        <div 
                                            style={{minHeight: '600px'}}
                                            className='bg-gray-50 w-5/6 border-4 mx-auto bg-boder-200 rounded'
                                        >{item.iframe}</div>
                                    }
                                </div>
                            ))
                        }
                        <div className='absolute animate-slide-right-to-left top-20 left-24 inline-flex items-center bg-gray-200'>
                            <div className="ipp-gome-image text-sm pl-2 py-2">
                                <img src="https://via.placeholder.com/50" alt="" />
                            </div>
                            <div className="text-title space-y-1 text-sm px-8 py-2">
                                <h3 className="font-semibold">This is the Title</h3>
                                <p className="text-content">This is the Content</p>
                            </div>
                        </div>
...
const adLabels = [
    { id: 'show', name: 'Show Ad Label', buttonLeft: true },
    { id: 'hide', name: 'Hide Ad Label', buttonRight: true }
]
...
const adPositions = [
    { id: 'top-right', name: 'Top-right', value: 'topRight', buttonLeft: true },
    { id: 'top-left', name: 'Top-Left', value: 'topLeft' },
    { id: 'bottom-right', name: 'Bottom-Right', value: 'bottomRight' },
    { id: 'bottom-left', name: 'Bottom-Left', value: 'bottomLeft', buttonRight: true }
]

const adReviewDevices = [
    {
        id: 'desktop',
        name: 'Desktop',
        icon: <><svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg></>,
        iframe: 
            <svg
                id="device-desktop"
                width="100%"
            ...
            >
                ...
                <path
                    d="M28 57.75H16M16 57.75L21.5 52M16 57.75L21.5 63"
                    stroke="#4B4D52"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
                <path
                    d="M0 6.4C0 4.15979 0 3.03969 0.435974 2.18404C0.819467 1.43139 1.43139 0.819467 2.18404 0.435974C3.03969 0 4.15979 0 6.4 0H884.6C886.84 0 887.96 0 888.816 0.435974C889.569 0.819467 890.181 1.43139 890.564 2.18404C891 3.03969 891 4.15979 891 6.4V40H0V6.4Z"
                    fill="#E1E4EB"
                />
                <path
                    d="M337 24H347M342 19V29"
                    stroke="#4B4D52"
                    strokeWidth={2}
                    strokeLinecap="round"
                    strokeLinejoin="round"
                />
                <path
                    d="M80 16C80 11.5817 83.5817 8 88 8H312C316.418 8 320 11.5817 320 16V40H80V16Z"
                    fill="white"
                />
                <rect
                    x={92}
                ...
                <defs>
                     <filter
                      ...
                      >
                        ...
                        <feColorMatrix
                          type="matrix"
                          values="0 0 0 0 0.937725 0 0 0 0 0.944105 0 0 0 0 0.956863 0 0 0 1 0"
                        />
                        <feBlend
                          mode="normal"
                    ...
                    >
                        <use
                          xlinkHref="#image0"
                          transform="translate(-0.195652) scale(0.00543478)"
                        />
                    </pattern>
                    <pattern
                        id="pattern1"
                        ...
                        height={1}
                    >
                        <use xlinkHref="#image1" transform="scale(0.00390625)" />
                    </pattern>
                    <pattern
                        id="pattern2"
                        patternContentUnits="objectBoundingBox"
                        ...
                        height={1}
                    >
                        <use xlinkHref="#image2" transform="scale(0.00333333)" />
                    </pattern>
                    <image
                        id="image0"
                        width={256}
    ...
    },
    {
        id: 'tablet',
        name: 'Tablet',
        icon: <><svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg></>,
        iframe: <svg
            id="device-tablet"
            width="100%"
            viewBox="0 0 891 77"
        ...
        >
            ...
            <pattern
                ...
                id="pattern1"
            >
                <use id="svg_2" transform="scale(0.00390625)" xlinkHref="#image1" />
            </pattern>
            <pattern
                height={1}
                width={1}
                ...
                id="pattern2"
            >
                <use id="svg_3" transform="scale(0.00333333)" xlinkHref="#image2" />
            </pattern>
            </defs>
            <g>
            <title>Layer 1</title>
            ...
            <path
                transform="rotate(90.8204 867.667 58)"
                id="svg_7"
                fill="#4B4D52"
                d="m869.16665,53c0,0.8284 -0.672,1.5 -1.5,1.5c-0.828,0 -1.5,-0.6716 -1.5,-1.5c0,-0.8284 0.672,-1.5 1.5,-1.5c0.828,0 1.5,0.6716 1.5,1.5zm0,5c0,0.8284 -0.672,1.5 -1.5,1.5c-0.828,0 -1.5,-0.6716 -1.5,-1.5c0,-0.8284 0.672,-1.5 1.5,-1.5c0.828,0 1.5,0.6716 1.5,1.5zm-1.5,6.5c0.828,0 1.5,-0.6716 1.5,-1.5c0,-0.8284 -0.672,-1.5 -1.5,-1.5c-0.828,0 -1.5,0.6716 -1.5,1.5c0,0.8284 0.672,1.5 1.5,1.5z"
                clipRule="evenodd"
                fillRule="evenodd"
            />
            <rect
            ...
            <path
                id="svg_15"
                fill="#4B4D52"
                d="m86,63.5c2.6961,0 4.9391,-1.9399 5.4093,-4.5l1.5198,0c-0.4852,3.3923 -3.4026,6 -6.9291,6c-3.866,0 -7,-3.134 -7,-7c0,-3.866 3.134,-7 7,-7c2.1877,0 4.141,1.0035 5.4246,2.5754l1.5754,-1.5754l0,5l-5,0l2.357,-2.357c-1.0057,-1.3034 -2.5833,-2.143 -4.357,-2.143c-3.0376,0 -5.5,2.4624 -5.5,5.5c0,3.0376 2.4624,5.5 5.5,5.5z"
            />
            ...
            <path
                ...
                strokeWidth="1.5"
                stroke="#A9AEB8"
                d="m48,57.75l12,0m0,0l-5.5,-5.75m5.5,5.75l-5.5,5.25"
            />
            ...
            <path
                ...
                strokeWidth="1.5"
                stroke="#4B4D52"
                d="m28,57.75l-12,0m0,0l5.5,-5.75m-5.5,5.75l5.5,5.25"
            />
            <path
                id="svg_20"
                fill="#E1E4EB"
                d="m0,6.4c0,-2.24021 0,-3.36031 0.43597,-4.21596c0.3835,-0.75265 0.99542,-1.36457 1.74807,-1.74807c0.85565,-0.43597 1.97575,-0.43597 4.21596,-0.43597l878.2,0c2.24,0 3.36,0 4.216,0.43597c0.753,0.3835 1.365,0.99542 1.748,1.74807c0.436,0.85565 0.436,1.97575 0.436,4.21596l0,33.6l-891,0l0,-33.6z"
            />
            <path
                id="svg_21"
                strokeLinejoin="round"
                ...
                strokeWidth={2}
                stroke="#4B4D52"
                d="m270,24l10,0m-5,-5l0,10"
            />
            <path
                id="svg_22"
                fill="white"
                d="m13,16c0,-4.4183 3.5817,-8 8,-8l224,0c4.418,0 8,3.5817 8,8l0,24l-240,0l0,-24z"
            />
            <rect
                id="svg_23"
                fill="url(#pattern2)"
            ...
            <g
                opacity="0.6"
                stroke="null"
                transform="matrix(0.0747336 0 0 0.0747336 307.083 31.5131)"
                id="svg_84"
            >
    ...
    },
    {
        id: 'mobile',
        name: 'Mobile',
        icon: <><svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg></>,
        iframe: <svg
            id="device-mobile"
            width="100%"
            viewBox="0 0 891 77"
        ...
        >
            <g>
                ...
                <path
                    ...
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="m308.125,36.18925l0,-0.50175l0,-1.875c0,-3.10669 -2.51813,-5.625 -5.625,-5.625c-3.10688,0 -5.625,2.51831 -5.625,5.625l0,1.875l0,0.50175c-1.12125,0.64838 -1.875,1.86019 -1.875,3.24825l0,5.625c0,2.07113 1.67813,3.75 3.75,3.75l7.5,0c2.07188,0 3.75,-1.67888 3.75,-3.75l0,-5.625c0,-1.38806 -0.75375,-2.59988 -1.875,-3.24825zm-9.375,-0.50175l1.875,0l3.75,0l1.875,0l0,-1.875c0,-2.07113 -1.67813,-3.75 -3.75,-3.75c-2.07188,0 -3.75,1.67888 -3.75,3.75l0,1.875z"
                    fill="#4B4D52"
                    id="svg_13"
                />
                <g stroke="null" id="svg_79" opacity="0.6">
                    <g
                        stroke="null"
                        id="svg_61"
                        transform="matrix(0.454811 0 0 0.484983 314.379 72.6679)"
                    >
                        <path
                            stroke="null"
                            d="m1166.18724,-80.93927l-10.112,0l0,6l10.112,0c3.007,0 5.34,1.536 5.34,2.858l0,26.606c0,1.322 -2.333,2.858 -5.34,2.858l-38.222,0c-3.007,0 -5.34,-1.536 -5.34,-2.858l0,-26.605c0,-1.322 2.333,-2.858 5.34,-2.858l10.11,0l0,-6l-10.11,0c-6.359,0 -11.34,3.891 -11.34,8.858l0,26.606c0,4.968 4.981,8.858 11.34,8.858l38.223,0c6.358,0 11.34,-3.891 11.34,-8.858l0,-26.606c-0.001,-4.968 -4.982,-8.859 -11.341,-8.859z"
                            id="svg_62"
                        />
                        <path
                            stroke="null"
                            d="m1138.19824,-89.33227c0.768,0 1.535,-0.293 2.121,-0.879l3.756,-3.756l0,13.028l0,6l0,11.494c0,1.657 1.343,3 3,3s3,-1.343 3,-3l0,-11.494l0,-6l0,-13.231l3.959,3.959c0.586,0.586 1.354,0.879 2.121,0.879s1.535,-0.293 2.121,-0.879c1.172,-1.171 1.172,-3.071 0,-4.242l-8.957,-8.957c-0.586,-0.586 -1.353,-0.877 -2.12,-0.877c-0.008,0 -0.015,0 -0.023,0s-0.015,0 -0.023,0c-0.767,0 -1.534,0.291 -2.12,0.877l-8.957,8.957c-1.172,1.171 -1.172,3.071 0,4.242c0.587,0.586 1.354,0.879 2.122,0.879z"
                            id="svg_63"
                        />
                    </g>
                    <g
                        stroke="null"
                        id="svg_64"
                        transform="matrix(0.454811 0 0 0.463072 314.379 74.5356)"
                    />
                    <g
                        stroke="null"
                        id="svg_65"
...
function IppDemo({ device, position }) {
    const [showPreview, setShowPreview] = useState(false)
    useEffect(() => {
        const intervalIpp = setInterval(() => { setShowPreview(!showPreview) }, 5000)
        return () => clearInterval(intervalIpp)
    }, [showPreview])   
    return (
        <div className={classNames(
            !showPreview ? 'hidden' : '',
            position === 'topRight' ? 'right-2 top-20 animate-slide-right-to-left' : '',
            position === 'bottomRight' ? 'right-2 bottom-10 animate-slide-right-to-left' : '',
            position === 'topLeft' ? 'left-2 top-20 animate-slide-left-to-right' : '',
            position === 'bottomLeft' ? 'left-2 bottom-10 animate-slide-left-to-right' : '',
            'absolute max-w-screen-lg bg-white text-gray-800 flex justify-center items-center shadow-lg rounded-lg'
        )}>
            <div className="ipp-gome-image text-sm pl-2 py-2">
                <img src="https://via.placeholder.com/50" alt="" />
            </div>
